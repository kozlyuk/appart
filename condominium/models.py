...
class Apartment(models.Model):
    ...
    def get_latest_bill(self, service):
        ...
        return self.bill_set.filter(billline__service=service) \
                            .latest('period')
    ...
    def order_number_generate(self, date):
        """ return autogenerated order number in format %Y%m%d-N """
        date_order_count = self.order_set.filter(date_created__contains=date) \
                                        .count() + 1
        return f'{date}-{self.account_number}-{date_order_count}'
